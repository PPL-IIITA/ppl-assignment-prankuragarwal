...
void makecouple::makeit2(boys boy[], girls girl[], int boys_num, int girls_num) {
	...
	*/
	for(i = 0; i < girls_num; i++){
		...
		fl = 0;
		idx = -1;
		if(girl[i].gcri == 1 && girl[i].committed > 1){		///most attractive
			for(j = 0; j < boys_num; j++){
				if(boy[j].committed != 1 && boy[j].committed != girl[i].committed){
					if(boy[j].bminreq <= girl[i].gattract && boy[j].bbudget >= girl[i].gmainbudget){
						if(boy[j].battract > ma){
							idx = j;
							ma = boy[j].battract;
		...
		} else if(girl[i].gcri == 2 && girl[i].committed > 1){ 		///most rich
				for(j = 0; j < boys_num; j++){
				if(boy[j].committed != 1 && boy[j].committed != girl[i].committed){
					if(boy[j].bminreq <= girl[i].gattract && boy[j].bbudget >= girl[i].gmainbudget){
						if(boy[j].bbudget > ma){
		...
		} else if(girl[i].gcri == 3 && girl[i].committed > 1){		///most intelligent
			for(j = 0; j < boys_num; j++){
				if(boy[j].committed != 1 && boy[j].committed != girl[i].committed){
					if(boy[j].bminreq <= girl[i].gattract && boy[j].bbudget >= girl[i].gmainbudget){
						if(boy[j].bintell > ma){
